...
function Sales() {
  ...
  const fetchProducts = async () => {
    try {
      const response = await fetch(`${SALES_API}/products`);
      const data = await response.json();
  ...
  const totalAmount = selectedProduct
    ? Number(selectedProduct.selling_price) * quantity
    : 0;

  const totalCost = selectedProduct
    ? Number(selectedProduct.purchase_price) * quantity
    : 0;
  ...
  return (
    <div className="page">
      <div className="page-header">
        <div>
          <h1>Sales</h1>
          <p>Record sales and track your profit</p>
        </div>
